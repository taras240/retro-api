(()=>{var v={date:(s,e)=>{let t=s.DateEarned?new Date(s.DateEarnedHardcore?s.DateEarnedHardcore:s.DateEarned):-1/0;return(e.DateEarned?new Date(e.DateEarnedHardcore?e.DateEarnedHardcore:e.DateEarned):-1/0)-t},earnedCount:(s,e)=>e.NumAwardedHardcore-s.NumAwardedHardcore,points:(s,e)=>parseInt(s.Points)-parseInt(e.Points),truepoints:(s,e)=>s.TrueRatio-e.TrueRatio,default:(s,e)=>s.DisplayOrder===0?v.id(s,e):s.DisplayOrder-e.DisplayOrder,id:(s,e)=>s.ID-e.ID,disable:(s,e)=>0,achievementsCount:(s,e)=>parseInt(s.NumAchievements)-parseInt(e.NumAchievements),title:(s,e)=>{let t=s.Title?.toUpperCase()??s.FixedTitle.toUpperCase(),a=e.Title?.toUpperCase()??e.FixedTitle.toUpperCase();return t<a?-1:t>a?1:0},award:(s,e)=>{let t={event:6,mastered:5,"beaten-hardcore":4,completed:3,"beaten-softcore":2,started:1},a=t[e.award]??0,i=t[s.award]??0,r=new Date(e.AwardedAt),n=new Date(s.AwardedAt);return a-i!=0?a-i:r-n}};var E=class{get _savedCompletionProgress(){return o._cfg?.apiWorker?.completionProgress??{}}get SAVED_COMPLETION_PROGRESS(){let e=this._savedCompletionProgress;return!e?.Total||o._cfg.apiWorker.targetUser!==o.targetUser?this.updateCompletionProgress({batchSize:500}).then(()=>this._savedCompletionProgress):this.updateCompletionProgress({batchSize:20,savedArray:e.Results}).then(()=>this._savedCompletionProgress)}set SAVED_COMPLETION_PROGRESS(e){e.Results=e.Results.map(t=>(delete t.ConsoleName,delete t.NumLeaderboards,t)),o._cfg.apiWorker||(o._cfg.apiWorker={}),o._cfg.apiWorker.targetUser=o.targetUser,o._cfg.apiWorker.completionProgress=e,o.writeConfiguration()}baseUrl="https://retroachievements.org/API/";endpoints={userProfile:"API_GetUserProfile.php",gameProgress:"API_GetGameInfoAndUserProgress.php",recentAchieves:"API_GetUserRecentAchievements.php",gameInfo:"API_GetGame.php",extendedGameInfo:"API_GetGameExtended.php",recentlyPlayedGames:"API_GetUserRecentlyPlayedGames.php",userAwards:"API_GetUserAwards.php",userGameRankAndScore:"API_GetUserGameRankAndScore.php",completionProgress:"API_GetUserCompletionProgress.php",gameList:"API_GetGameList.php",userSummary:"API_GetUserSummary.php",wantToPlay:"API_GetUserWantToPlayList.php"};async getLastUnlocks({apiKey:e,targetUser:t}){let a=new URL(this.endpoints.recentAchieves,this.baseUrl),i={y:e||o.API_KEY,u:t||o.targetUser,m:1440*31};return a.search=new URLSearchParams(i),(await fetch(a).then(l=>l.json()))?.map(l=>({...l,ID:l.AchievementID,DateEarned:l.Date})).sort((l,c)=>v.date(l,c))??[];return[{ID:383065,GameID:1891,GameTitle:"Puzznic",Title:"Enigma Explorer",Description:"Reach a score of 50,000 in Puzznic",Points:2,Type:null,BadgeName:"432937",IsAwarded:"1",DateAwarded:"2026-06-05 17:13:14",HardcoreAchieved:1},{ID:383073,GameID:1891,GameTitle:"Puzznic",Title:"Puzznic Level 1 - Riddle Ridge",Description:"In a single session, complete all of the Puzznic puzzles on level 1 [No continues]",Points:3,Type:"progression",BadgeName:"432901",IsAwarded:"1",DateAwarded:"2026-06-05 17:11:57",HardcoreAchieved:1}]}getUrl({endpoint:e,targetUser:t,gameID:a,minutes:i,apiKey:r,userName:n,achievesCount:l,count:c,offset:m}){let p=new URL(e,this.baseUrl),h={y:r||o.API_KEY,z:n||o.USER_NAME,u:t||o.targetUser,g:a||o.gameID,m:i||2e3,i:a||o.gameID,f:1,h:1,a:l||5,c:c||20,o:m||0};return p.search=new URLSearchParams(h),p}constructor(){}getUserCompelitionProgress({targetUser:e,count:t,offset:a}){let i=this.getUrl({targetUser:e||o.targetUser,count:t||100,offset:a||0,endpoint:this.endpoints.completionProgress});return fetch(i).then(r=>r.json()).then(r=>(r.Results=r.Results.map(n=>(n.ID=n.GameID,n.NumAchievements=n.MaxPossible,delete n.MaxPossible,delete n.NumLeaderboards,n)),r))}getUserAwards({targetUser:e}){let t=this.getUrl({targetUser:e||o.targetUser,endpoint:this.endpoints.userAwards});return fetch(t).then(a=>a.json()).then(a=>(a.VisibleUserAwards=a.VisibleUserAwards.map(i=>["Mastery/Completion","Game Beaten"].includes(i.AwardType)?(i.award=i.AwardType=="Game Beaten"?i.AwardDataExtra=="1"?"beaten":"beaten_softcore":i.AwardDataExtra=="1"?"mastered":"completed",i.DateEarned=i.AwardedAt,i.ConsoleName=="Events"&&(i.award="event"),i.timeString=this.toLocalTimeString(i.AwardedAt),i=this.fixGameTitle(i),i):null).filter(i=>i),a))}getGameProgress({targetUser:e,gameID:t}){let a=this.getUrl({endpoint:this.endpoints.gameProgress,targetUser:e||o.targetUser,gameID:t||o.gameID});return fetch(a).then(i=>i.json()).then(i=>{i={...i,TotalRealPlayers:0,TotalRetropoints:0,points_total:0,progressionRetroRatio:0,beatenCount:1/0,masteredCount:1/0,earnedStats:{soft:{count:0,points:0,retropoints:0},hard:{count:0,points:0,retropoints:0}}};let r={Count:0,WinCount:0,WinAwardedCount:0,WinEarnedCount:0},n={isBeaten:!0,isBeatenSoftcore:!0,isWinEarned:!1,isWinEarnedSoftcore:!1};for(let c of Object.values(i.Achievements))i.TotalRetropoints+=c.TrueRatio,i.points_total+=c.Points,i.TotalRealPlayers<c.NumAwarded&&(i.TotalRealPlayers=c.NumAwarded),c.DateEarned&&(i.earnedStats.soft.count+=1,i.earnedStats.soft.points+=c.Points,i.earnedStats.soft.retropoints+=c.TrueRatio),c.DateEarnedHardcore&&(i.earnedStats.hard.count+=1,i.earnedStats.hard.points+=c.Points,i.earnedStats.hard.retropoints+=c.TrueRatio),c.type==="progression"&&(r.Count++,c.DateEarned?c.DateEarnedHardcore||(n.isBeaten=!1):(n.isBeaten=!1,n.isBeatenSoftcore=!1),i.beatenCount>c.NumAwardedHardcore&&(i.beatenCount=c.NumAwardedHardcore)),c.type==="win_condition"&&(c.DateEarnedHardcore?(n.isWinEarned=!0,n.isWinEarnedSoftcore=!0):c.DateEarned&&(n.isWinEarnedSoftcore=!0),r.WinCount++,c.NumAwardedHardcore>r.WinAwardedCount&&(r.WinAwardedCount=c.NumAwardedHardcore),c.DateEarnedHardcore&&r.WinEarnedCount++),c.NumAwardedHardcore<i.masteredCount&&(i.masteredCount=c.NumAwardedHardcore);i.achievements_published==i.NumAwardedToUserHardcore?i.award="mastered":n.isBeaten&&(n.isWinEarned||r.WinCount==0)&&(i.award="beaten"),i={...i,winVariantCount:r.WinCount,winEarnedCount:r.WinEarnedCount,progressionSteps:r.WinCount>0?r.Count+1:r.Count},r.WinCount>0&&(i.beatenCount=r.WinAwardedCount),i.beatenCount!=1/0&&(i.beatenRate=~~(1e4*i.beatenCount/i.TotalRealPlayers)/100),i.masteredCount!=1/0&&(i.masteryRate=~~(1e4*i.masteredCount/i.TotalRealPlayers)/100);let l=~~(i.TotalRetropoints/i.points_total*100)/100;return i.retroRatio=l,i.gameDifficulty=l>9?"insane":l>7?"expert":l>5?"pro":l>3?"standard":"easy",Object.values(i.Achievements).map(c=>this.fixAchievement(c,i)),i=this.fixGameTitle(i),i})}getGameInfo({gameID:e,extended:t}){let a=this.getUrl({endpoint:this.endpoints[t?"extendedGameInfo":"gameInfo"],gameID:e});return fetch(a).then(i=>i.json())}getUserProfile({userName:e}){let t=this.getUrl({targetUser:e,userName:e,endpoint:this.endpoints.userProfile});return fetch(t).then(a=>a.json())}getUserSummary({targetUser:e,gamesCount:t=3,achievesCount:a}){let i=this.getUrl({targetUser:e,gameID:t,achievesCount:a,endpoint:this.endpoints.userSummary});return fetch(i).then(r=>r.json()).then(r=>(r.RecentlyPlayed=r.RecentlyPlayed.map(n=>(n.LastPlayed=this.toLocalTimeString(n.LastPlayed),r.Awarded[n.GameID]&&(n={...n,...r.Awarded[n.GameID]}),n=this.fixGameTitle(n),n)),r.RecentAchievements=Object.values(r.RecentAchievements).flatMap(n=>Object.values(n)).map(n=>(n.DateEarned=this.toLocalTimeString(n.DateAwarded),n)),r.isInGame=new Date-new Date(r.RecentlyPlayed[0].LastPlayed)<300*1e3,r))}verifyUserIdent({userName:e,apiKey:t}){let a=this.getUrl({targetUser:e,userName:e,apiKey:t,endpoint:this.endpoints.userProfile});return fetch(a).then(i=>i.json())}getGameList({userName:e,apiKey:t,systemID:a}){let i=this.getUrl({userName:e,apiKey:t,gameID:a,endpoint:this.endpoints.gameList});return fetch(i).then(r=>r.json())}doTestEndpoint({endpoint:e}){let t=this.getUrl({endpoint:e});return fetch(t).then(a=>a.json()).then(a=>console.log(a))}async updateCompletionProgress({savedArray:e=[],completionProgress:t=[],batchSize:a=500}){let i=await this.getUserCompelitionProgress({count:a,offset:t.length});t=[...t,...i.Results];let r=t.at(-1);if(e.findIndex(l=>l.hasOwnProperty("GameID")&&l.GameID===r.GameID&&l.MostRecentAwardedDate===r.MostRecentAwardedDate)>=0||t.length===i.Total){let l=t.map(c=>c.GameID);e=e.filter(c=>!l.includes(c.GameID)),e=[...t,...e],this.SAVED_COMPLETION_PROGRESS={Total:e.length,Results:e}}else setTimeout(()=>this.updateCompletionProgress({savedArray:e,completionProgress:t,batchSize:a}),100)}fixAchievement(e,t){let{BadgeName:a,DateEarned:i,DateEarnedHardcore:r,NumAwardedHardcore:n,NumAwarded:l,TrueRatio:c,ID:m}=e,{NumDistinctPlayers:p,NumAwardedToUserHardcore:h,TotalRealPlayers:f}=t,w=100*(n-h*.5)/((p+f)*.5-h*.5);t.Achievements[m]={...e,totalPlayers:p,isEarned:!!i,isHardcoreEarned:!!r,DateEarned:i&&this.toLocalTimeString(i),DateEarnedHardcore:r&&this.toLocalTimeString(r),prevSrc:`https://media.retroachievements.org/Badge/${a}.png`,rateEarned:~~(100*l/p)+"%",rateEarnedHardcore:~~(100*n/p)+"%",trend:w,difficulty:w<1.5&&c>300||c>=500?"hell":w<=3&&c>100||c>=300?"insane":w<8&&c>24?"expert":w<13&&c>10?"pro":w<20&&c>5||c>10?"standard":"easy"}}fixGameTitle(e){let t=[/\[SUBSET[^\[]*\]/gi,/~[^~]*~/g,".HACK//"],a=e.Title,i=t.reduce((r,n)=>{let l=new RegExp(n,"gi"),c=e.Title?.match(l);return c&&c.forEach(m=>{a=a.replace(m,"");let p=m;r.push(p.replace(/[~\.\[\]]|subset -|\/\//gi,""))}),r},[]);return e.badges=i,e.FixedTitle=a?.trim(),e}toLocalTimeString(e){!/(\+00\:00$)|(z$)/gi.test(e)&&(e+="+00:00");let a=new Date(e),i={day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit",hour12:!1};return a}async getWantToPlayGamesList({apiKey:e,targetUser:t,count:a,offset:i}){let r=new URL(this.endpoints.wantToPlay,this.baseUrl),n={y:e||o.API_KEY,u:t||o.targetUser,o:i||0};return r.search=new URLSearchParams(n),(await fetch(r).then(c=>c.json())).Results||[]}};var U="retroApiConfig",I=class{get version(){return this._cfg.version??"0"}set version(e){this._cfg.version=e,this.writeConfiguration()}get API_KEY(){return this._cfg.identification.RAApi_key}set API_KEY(e){this._cfg.identification.RAApi_key=e,this.writeConfiguration()}get USER_NAME(){return this._cfg.identification.RAApi_login}set USER_NAME(e){this._cfg.identification.RAApi_login=e,this.writeConfiguration()}get identConfirmed(){return this._cfg.identification.identConfirmed??!1}set identConfirmed(e){this._cfg.identification.identConfirmed=e,this.writeConfiguration()}get userImageSrc(){return this._cfg.identification.userImageSrc||""}set userImageSrc(e){this._cfg.identification.userImageSrc=e,this.ui.buttons&&(ui.buttons.userImage.src=e),this.writeConfiguration()}get targetUser(){return this._cfg.settings.targetUser||this.USER_NAME}set targetUser(e){this._cfg.settings.targetUser=e,this.writeConfiguration(),this.identConfirmed&&(ui.settings.getLastGameID(),ui.awards.updateAwards())}get gameID(){return this._cfg.settings.gameID}set gameID(e){this._cfg.settings.gameID=e,this.writeConfiguration()}get ui(){return this._cfg.ui}constructor(){this.readConfiguration()}readConfiguration(){let e=JSON.parse(localStorage.getItem(U));e||(e={identification:{RAApi_key:"",RAApi_login:""},settings:{updateDelay:15,sort:"default",gameID:1},ui:{}}),this._cfg=e,localStorage.setItem(U,JSON.stringify(this._cfg)),this.writeConfiguration()}delayedWrite;writeConfiguration(){clearTimeout(this.delayedWrite),this.delayedWrite=setTimeout(()=>{localStorage.setItem(U,JSON.stringify(this._cfg))},1e3)}};function b(s){return s?.reduce((e,t)=>e+=`<i class="game-badge game-badge__${t.toLowerCase()}">${t}</i>`,"")}var W={points:`
        ...
        </svg>`};function L(s){return new Date(s).toLocaleString()}var u=s=>{/\/>/g.test(s)&&(s=s.replace(/<(\w+)([^>]*)\/>/g,(t,a,i)=>new Set(["area","base","br","col","embed","hr","img","input","link","meta","param","source","track","wbr"]).has(a.toLowerCase())?t:`<${a}${i}></${a}>`));let e=document.createElement("template");return e.innerHTML=s.trim(),e.content.firstElementChild};function q(s){let e=+(s.retropoints/s.hardpoints).toFixed(2);return`
    ...
    `}var $=({BadgeName:s})=>`https://media.retroachievements.org/Badge/${s}.png`,S=s=>`https://media.retroachievements.org${s}`;function N(s,e={}){let{Title:t,Description:a,ID:i,GameID:r,HardcoreAchieved:n,IsAwarded:l,DateEarned:c,BadgeName:m,Points:p,DateEarnedHardcore:h}=s,f=n||c?"unlocked":"locked",w=u(`
                    <div class="achievement ${f}">
                        <div class="ach-icon">
                            <img class="ach-img" src="${$({BadgeName:m})}"/>
                        </div>
                        <div class="ach-info">
                ...
                `);return`
                    <div class="achievement ${f}" onclick="ui.showAchivDetails(${i}, ${r}); event.stopPropagation()">
                        <div class="ach-icon">
                            <img class="ach-img" src="${$({BadgeName:m})}"/>
                        </div>
                        <div class="ach-info">
                ...
                `}function z({lastAchievements:s}){return s.reduce((e,t)=>{let a=N(t);return e+=a,e},"")}var ae=s=>{let e=new Date(s),t=e.getTime(),a=Date.now(),i=Math.round((a-e)/6e4);if(i<2)return"just now";if(i<60)return i+" minutes ago";let r=Math.round(i/60);return r<24?r+" hours ago":e.toLocaleString()};function ie(s){let{GameID:e,ImageIcon:t,FixedTitle:a,badges:i,LastPlayed:r,ConsoleName:n,NumAchieved:l,NumAchievedHardcore:c,NumPossibleAchievements:m,ScoreAchieved:p,ScoreAchievedHardcore:h,PossibleScore:f}=s;return`    
        <li class="user-info__last-game-container" data-id="${e}">
            <div class="user-info__game-main-info"  onclick="ui.showGameDetails(${e}); event.stopPropagation()">
                ...
                <div class="user-info__game-description" >
                    <h2 class="user-info__game-title">${a} ${b(i)}</h2>
                    <div class="game-stats__text">${L(r)} | ${n}</div>
  ...
  `,t}};var K={earned:s=>!!s.DateEarnedHardcore,notEarned:s=>!s.DateEarnedHardcore,missable:s=>s.type==="missable",progression:s=>s.type==="progression"||s.type==="win_condition",all:()=>!0};function O(s,e=!1){if(!s)return"-";if(s=Number(s),e)return s>=3600?`${Math.round(s/3600)}h`:s>=60?`${Math.round(s/60)}min`:`${Math.round(s)}s`;let t=Math.floor(s/3600),a=Math.floor(s%3600/60),i=Math.floor(s%60);return t?`${t}h ${a}m`:a?`${a} min${a>1?"s":""}`:`${i} secs`}var P=class{filterContext=()=>({label:"Filter by",elements:[{label:"Progression",id:"filter_progression",type:"radio",onChange:"ui.game.filter = 'progression'",checked:this.filter==="progression",name:"filter-by-progression"},{label:"Missable",id:"filter_missable",type:"radio",onChange:"ui.game.filter = 'missable'",checked:this.filter==="missable",name:"filter-by-missable"},{label:"Earned",id:"filter_earned",type:"radio",onChange:"ui.game.filter = 'earned'",checked:this.filter==="earned",name:"filter-by-earned"},{label:"Disable",id:"filter_all",type:"radio",onChange:"ui.game.filter = 'all'",checked:this.filter==="all",name:"filter-by-all"},{label:"Reverse filter",id:"filter_reverse-filter",type:"checkbox",onChange:"ui.game.filterReverse = this.checked",checked:this.filterReverse==!0,name:"filter-cheevos-reverse"}]});sortContext=()=>({label:"Sort by",elements:[{label:"Earned date",id:"sort_latest",type:"radio",onChange:"ui.game.sortType = 'date'; ",checked:this.sortType==="date",name:"sort-cheevos"},{label:"Points",id:"sort_points-count",type:"radio",onChange:"ui.game.sortType = 'points'",checked:this.sortType==="points",name:"sort-cheevos"},{label:"Retropoits",id:"sort_retropoints-count",type:"radio",onChange:"ui.game.sortType = 'truepoints'",checked:this.sortType==="truepoints",name:"sort-cheevos"},{label:"Rarity",id:"sort_rarity-count",type:"radio",onChange:"ui.game.sortType = 'earnedCount'",checked:this.sortType==="earnedCount",name:"sort-cheevos"},{label:"Default",id:"sort_default-count",type:"radio",onChange:"ui.game.sortType = 'default'",checked:this.sortType==="default",name:"sort-cheevos"},{label:"Reverse sort",id:"sort_reverse-sort",type:"checkbox",onChange:"ui.game.sortTypeReverse = this.checked",checked:this.sortTypeReverse==-1,name:"sort-cheevos-reverse"}]});cheevosListContext=()=>({label:"Show by",elements:[...Object.getOwnPropertyNames(this.listTypes).reduce((e,t)=>{let a={label:`${this.listTypes[t]}`,id:`game_list-type-${t}`,type:"radio",onChange:`ui.game.listType = '${t}'`,checked:this.listType==t,name:"game-list-type"};return e.push(a),e},[])]});get sortType(){return o.ui?.mobile?.game?.sortType??"title"}set sortType(e){o.ui.mobile.game.sortType=e,o.writeConfiguration(),this.updateCheevos()}get filter(){return o.ui?.mobile?.game?.filter??"all"}set filter(e){o.ui.mobile.game.filter=e,o.writeConfiguration(),this.updateCheevos()}get sortTypeReverse(){return o.ui?.mobile?.game?.sortTypeReverse??1}set sortTypeReverse(e){o.ui.mobile.game.sortTypeReverse=e?-1:1,o.writeConfiguration(),this.updateCheevos()}get filterReverse(){return o.ui?.mobile?.game?.filterReverse??!1}set filterReverse(e){o.ui.mobile.game.filterReverse=e,o.writeConfiguration(),this.updateCheevos()}get listType(){return o.ui?.mobile?.game.listType??"grid"}set listType(e){o.ui.mobile.game.listType=e,o.writeConfiguration(),this.update()}listTypes={list:"list",grid:"grid"};constructor(e){!o.ui.mobile.game&&(o.ui.mobile.game={}),this.gameID=e,this.update()}updateCheevos(){this.achievements=Object.values(g[this.gameID].Achievements),this.achievements=this.achievements.filter(t=>this.filterReverse^K[this.filter](t)),this.achievements=this.achievements.sort((t,a)=>this.sortTypeReverse*v[this.sortType](t,a));let e=document.querySelector(".game-achivs__container");e.innerHTML=this.AchievementsListHtml()}getSectionElement(){let e=document.createElement("div");return e.classList.add("game__section","section"),e.innerHTML=`
            ...
            <div class="game-header__main-info">
                ...
                <div class="game-header__description-container">
                    <h1 class="game-header__title">
                     ${this.gameData.FixedTitle} ${b(this.gameData.badges)}                       
                    </h1>
    ...
    `}update(e=this.gameID){if(d.showLoader(),g[e]){this.gameData=g[this.gameID],this.achievements=Object.values(g[this.gameID].Achievements);let t=this.generateGameSection(this.gameData);d.content.innerHTML="",d.content.append(t),this.updateGameSection(this.gameData),d.removeLoader()}else _.getGameProgress({gameID:e}).then(t=>{g[e]=t}).then(()=>this.update())}updateGameSection(e){let t=r=>{let{ImageIcon:n,Title:l,ConsoleName:c,Developer:m,UserTotalPlaytime:p,TotalPoints:h}=r;document.querySelector(".game-image").src=S(n),document.querySelector(".game-title").innerHTML=l,document.querySelector(".game-meta>.platform").innerHTML=c,document.querySelector(".game-meta>.game-dev").innerHTML=m,document.querySelector(".playtime").innerHTML="Time played: "+O(p,!1)},a=r=>{let{NumAchievements:n,NumAwardedToUserHardcore:l,points_total:c}=r,m=r.earnedStats?.hard?.points??0,p=Math.floor(100*l/n);document.querySelector(".progress-card").style.setProperty("--unlock-rate",`${p}%`),document.querySelector(".progress-card .progress-count").innerHTML=`${l} / ${n}`,document.querySelector(".progress-footer .points").innerHTML=`${m} / ${c} points`,document.querySelector(".progress-card .progress-footer>span").innerHTML=`${p}% completed`},i=r=>{function n(c,m){let{Title:p,Description:h,Points:f,BadgeName:w,DateEarnedHardcore:ee}=c,B=u(`
                    <div class="achievement ${ee?"unlocked":"locked"}">
                        <div class="ach-icon">
                            <img class="ach-img" src="${$({BadgeName:w})}"/>
                        </div>
                        <div class="ach-info">
                            <div class="ach-name">${p}</div>
                            <div class="ach-desc">${h}</div>
                        </div>
                        <div class="ach-points">${f}</div>
                    </div>
                `);return B.addEventListener("click",te=>{te.stopPropagation(),d.showAchivDetails(c.ID,m.ID),console.log(m)}),B}let l=document.querySelector(".achievement-list");l.innerHTML="",Object.values(r.Achievements).sort((c,m)=>v.date(c,m)).forEach(c=>{let m=n(c,r);l.append(m)})};t(e),a(e),i(e)}generateGameSection(e){let t=()=>{let n=u(`
            ...
            `);return n.addEventListener("click",l=>{l.stopPropagation(),d.showGameDetails(e.ID)}),n},a=()=>u(`
        ...
        `);return r.append(t(),a(),i()),r}};var J=["ID","Title","badges","ConsoleID","ImageIcon","NumAchievements","Points","retropoints","relisedAt","timeToBeat","timeToMaster","playersHardcore","timesBeaten","timesMastered","playersTotal","genres","series","updated"];var se="./json/games/all_min.json",re=s=>s.map(t=>{let a={};if(t.forEach((i,r)=>{a={...a,[J[r]]:i}}),a.timeToBeat){let i=Math.round(a.timeToBeat/60),r=i>=60?`${~~(i/60)}hr${i>119?"s":""}`:"",n=i%60>0?`${i%60}mins`:"",l=`${r} ${n}`;a.timeToBeatString=l}return a.Date=a.relisedAt?new Date(a.relisedAt).toLocaleDateString():"",a.ModifiedDate=a.updated?new Date(a.updated).toLocaleDateString():"",a.trueRatio=+(a.retropoints/a.Points).toFixed(1),a.beatenRate=Number((100*a.timesBeaten/a.playersHardcore).toFixed(1))||0,a.masteryRate=Number((100*a.timesMastered/a.playersHardcore).toFixed(1))||0,a.ImageIcon=`/Images/${a.ImageIcon}.png`,a.badges??=[],a}),Y=async(s=se)=>{let t=await(await fetch(s)).json();return re(t)};function X(s){return u(`
        <div class="section__header-container">
    ...
    `)}function F(s){let{ID:e,ImageIcon:t,Title:a,badges:i,NumAchievements:r,Points:n,ConsoleID:l}=s,c=u(`
        <li class="library__game-item">
            <div class="library__game-container"  onclick="ui.showGameDetails(${e}); event.stopPropagation()">
                ...
                <div class="awards__game-description">
                    <h2 class="library__game-title">${a} ${b(i)}</h2>
                    <div  class="game-stats__button"  onclick="ui.expandGameItem(${e},this); event.stopPropagation()">
                        <i class="game-stats__icon game-stats__expand-icon"/>
                    </div>
                    <div class="awards__game-stats__text">${y[l]}</div>
    ...
    `,e}getGameElement(e){let t=document.createElement("li");return t.classList.add("awards__game-item"),t.dataset.id=e?.ID,t.innerHTML=`
        ...
        `,t}};var Q=()=>fetch("./sections/login.elem").then(e=>e.text()).then(e=>{let t=document.createElement("section");return t.className="login__section",t.innerHTML=e,t}).then(e=>(o.USER_NAME&&(e.querySelector("#login_user-name").value=o.USER_NAME),o.API_KEY&&(e.querySelector("#login__api-key").value=o.API_KEY),o.identConfirmed&&e.querySelector("#login__submit-button").classList.add("verified"),e));var Z=()=>{let s=document.createElement("div");return s.classList.add("loading_screen"),s.innerHTML='<div class="loading_screen__loader-icon"></div>',s};var H=class{constructor(){this.update()}getSectionElement(){return u(`
      ...
      <a class="round-button icon-button download-icon simple-button game-popup__download-button"
          href="https://google.com/search?q='${e?.FixedTitle}' '${y[e?.ConsoleID]}' ${ne}" target="_blank"></a>
    ...
    `}async showAchivDetails(e,t){if(this.removePopups(),this.showLoader(),g[t]){let a=document.createElement("div");a.addEventListener("touchend",n=>n.stopPropagation()),a.classList.add("popup-info__container","popup");let i=g[t].Achievements[e],r=this.achivPopupHtml(i);a.innerHTML=r,this.content.append(a),this.removeLoader()}else{let a=await _.getGameProgress({gameID:t});g[t]=a,this.showAchivDetails(e,t)}}achivPopupHtml(e){return`
    ...
    <div class="popup-info__properties">
        ...
        ${e?.isEarned?`<div class="popup-info__property">Date earned : <span>${L(e?.DateEarned)}</span></div>`:""}
        ${e?.isHardcoreEarned?`<div class="popup-info__property">Date earned hardcore: <span>${L(e?.DateEarnedHardcore)}</span></div>`:""}
  ...
  `}expandGameItem(e,t){let a=t.closest("li");a.classList.toggle("expanded");let i=r=>{this.showLoader();let n=u(`
      ...
      `),l=n.querySelector("ul");a.appendChild(n),g[r]?(Object.values(g[r].Achievements).sort((c,m)=>v.date(c,m)).forEach(c=>{l.innerHTML+=this.achivHtml(c,r)}),this.removeLoader()):_.getGameProgress({gameID:r}).then(c=>{g[r]=c,Object.values(c.Achievements).sort((m,p)=>v.date(m,p)).forEach(m=>{l.innerHTML+=this.achivHtml(m,r)})}).then(()=>this.removeLoader())};a.querySelector(".user-info__game-achivs-container")??i(e)}achivHtmlList(e,t){return`    
        ...
        `}showLoader(){this.removeLoader(),this.app.append(Z())}removeLoader(){document.querySelectorAll(".loading_screen").forEach(e=>e.remove())}},oe=()=>fetch("./sections/test.elem").then(e=>e.text()).then(e=>{let t=document.createElement("section");return t.className="test__section section",t.innerHTML=e,t});var ne="site:www.romhacking.net OR site:wowroms.com/en/roms OR site:cdromance.org OR site:coolrom.com.au/roms OR site:planetemu.net OR site:emulatorgames.net OR site:romsfun.com/roms OR site:emu-land.net/en";function j(s,e){e.stopPropagation(),ui.removeContext();let t=document.createElement("div");t.classList.add("context-menu__container","context"),t.addEventListener("touchend",r=>r.stopPropagation()),t.addEventListener("mousedown",r=>r.stopPropagation()),t.innerHTML+=`
  ...
  `;let a=()=>{let r=document.createElement("div");return r.classList.add("context__controls"),s.elements.forEach(n=>{switch(n.type){case"radio":r.innerHTML+=`
          ...
          `;break;default:return""}}),r},i=document.createElement("div");i.classList.add("context__control-container"),i.append(a(s)),t.append(i),ui.app.appendChild(t)}var o=new I,_=new E,d=new M;window.ui=d;window.generateContextMenu=j;window.submitRAData=()=>{let s=d.content.querySelector("#login_user-name").value,e=d.content.querySelector("#login__api-key").value;_.verifyUserIdent({userName:s,apiKey:e}).then(t=>{if(t.ID)ce({userName:s,apiKey:e,userObj:t}),setTimeout(()=>{d.goto.home(),location.reload(!0)},1e3);else{o.identConfirmed=!1;let a=d.content.querySelector("#login__submit-button");a.classList.add("error"),a.classList.remove("verified")}})};function ce({userName:s,apiKey:e,userObj:t}){o.USER_NAME=s,o.API_KEY=e,o.identConfirmed=!0,o.userImageSrc=`https://media.retroachievements.org${t?.UserPic}`;let a=d.content.querySelector("#login__submit-button");a.classList.remove("error"),a.classList.add("verified")}})();
