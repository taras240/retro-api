...
export const buttonsHtml = {
    ...
    comments: (onClick, classes = []) => `<button class="comments-button header-button header-icon chat-icon ${classes.join(" ")}" data-title="${lang.showCommentsHint}" onclick="${onClick}"></button>`,
    ...
    saveData: ({ hint, id, className }) => `<button id="${id || getRandomID()}" class="header-icon header-button save-icon ${className ?? ""}" data-title="${hint ?? lang.saveData}"></button>`,
    editGameProps: (onClick) => editPropsButtons(true),
    editCheevoProps: (cheevoID, onClick) => editPropsButtons(false, cheevoID, onClick),

    tweek: (onClick) => `<button class="header-button header-icon tweak-button tweak-icon" data-title="${lang.widgetConfigHint}" onclick="${onClick}"></button>`,

    removeFromTarget: () => `<button class="header-button header-icon  delete-icon delete-from-target" data-title="${lang.removeFromTargetHint}"></button>`,
    pin: () => `<button class="header-button header-icon bookmark-icon pin-cheevo" data-title="${lang.addPin}"></button>`,
